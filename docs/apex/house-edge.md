...
## 6. Tier S 게임 (Phase 3 예약)

본 절은 Phase 3 출시 예정인 Tier S 게임 5종의 페이아웃 수식 / RTP 타깃 / 변동성 등급을
사전 동결한다 (구현은 Phase 3, 수식은 본 문서로 고정).

| 게임   | RTP    | House Edge | Variance | Max Multiplier | Min Bet (PHON) | 비고                          |
| ------ | ------ | ---------- | -------- | -------------- | -------------- | ----------------------------- |
| Pump   | 99.00% | 1.00%      | Med-High | 25,000x        | 100            | 인플레이션 곡선, instant cash |
| Wheel  | 99.00% | 1.00%      | Low-High | 50x            | 50             | 3 risk × 10/20/30 segs        |
| Limbo  | 99.00% | 1.00%      | High     | 1,000,000x     | 10             | target multiplier 입력형      |
| Keno   | 95.00% | 5.00%      | High     | 10,000x        | 100            | 40개 중 10 픽 (BC.Game 동급)  |
| HiLo   | 99.00% | 1.00%      | Low-Med  | 10,000x        | 50             | 카드 기반, 누적 multiplier    |

### 6.1 Pump
- Crash와 동일 분포 (`P(X≥x)=0.99/x`) 위에서 사용자가 임의 시점 cashout.
- 인플레이션 곡선 = `m(t) = 1 + e^(0.07·t) − 1` (시각 효과, 분포 비변경).

### 6.2 Wheel
- 세그먼트 가중치 정규화 `Σ p_i·m_i = 0.99`. Risk={low, med, high} × Segments={10,20,30}.

### 6.3 Limbo
- target `c ≥ 1.01` 입력. `payout = c`, 승률 = `0.99/c`. Max c = 1,000,000.

### 6.4 Keno
- 40개 중 10픽, hit count 별 페이아웃 테이블 정규화 → RTP 95.00% (Keno 업계 표준).

### 6.5 HiLo
- 카드 13장 deck, 각 픽 `payout_next = 0.99 / p_correct`. 누적 multiplier = Π payouts.

### 6.6 Stake.com / Rollbit / BC.Game Tier S 비교

| 게임   | Stake.com  | Rollbit    | BC.Game     | ApexForge  | 우위         |
| ------ | ---------- | ---------- | ----------- | ---------- | ------------ |
| Pump   | 99.00%     | 98.50%     | 99.00%      | 99.00%     | =            |
| Wheel  | 96.0~99.0% | 96.0%      | 97.0~99.0%  | 99.00%     | ApexForge ✅ |
| Limbo  | 99.00%     | 99.00%     | 99.00%      | 99.00%     | =            |
| Keno   | 91.0~95.0% | 94.0%      | 95.0%       | 95.00%     | =            |
| HiLo   | 99.00%     | 98.50%     | 99.00%      | 99.00%     | =            |

> Tier S 5종 시뮬레이션은 `scripts/slot-sim.ts` (100M spin) 결과로 Phase 3 출시 시 본 문서에 추가.
...
| 2026-05-20 | Tier S 5종(Pump/Wheel/Limbo/Keno/HiLo) 수식 동결  | (문서만, Phase 3 예약)   |
