...
BEGIN
  ...
  ELSIF _game_code = 'plinko' THEN
    -- RTP 정규화: scale = 0.99 / mean(payout_array)
    --   low    mean = 65.7/17  = 3.8647 → scale 0.256166
    --   medium mean = 344.3/17 = 20.2529 → scale 0.048882
    --   high   mean = 2343/17  = 137.8235 → scale 0.007183
    ...
    BEGIN
      ...
      END;
      v_scale := CASE v_risk
        WHEN 'low'  THEN 0.256166
        WHEN 'high' THEN 0.007183
        ELSE             0.048882
      END;
      v_mult := round((v_pay[v_bin+1] * v_scale)::numeric, 4);
      v_result := jsonb_build_object('bin',v_bin,'risk',v_risk,'base',v_pay[v_bin+1]);
    END;
